...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-1.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-2.png)

根据需求简单选一下功能，然后编译打包下载。

编译成功后直接下载镜像。一般直接下载`EFI.img.gz`结尾的文件即可。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-3.png)

因为涉及到的基础知识比较多，我不太清楚哪里需要展开，哪里可以带过。如果有不懂的地方，==欢迎留言提问==。我将在第一时间回复，并在后续文章中补充。

### 2. 上传固件文件到nas

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-4.png)

直接在飞牛 OS 的文件管理器中上传刚才下载的固件文件。或者通过 Samba、FTP 等方式上传也可以。

### 3. 创建OpenWrt虚拟机

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-5.png)

打开飞牛 OS 的`虚拟机`，点击`新建虚拟机`。没有的话去`应用中心`下载一个。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-6.png)

给虚拟机取个名字，操作系统选`Linux`，版本随便选。然后点下一步

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-7.png)

系统镜像选择刚才上传的固件文件，其他的按自己需求来，基本上是不需要改的。==开机启动建议选择是==。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-8.png)

存储空间可以随意选择。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-9.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-10.png)

硬件直通直接忽略，点下一步。这样你的虚拟机就创建好了。

### 4. 启动OpenWrt虚拟机

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-11.png)

刚创建好的虚拟机会是关机状态，点击开关按钮，然后点击`开机`即可。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-12.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-13.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-14.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-15.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-16.png)

第一次访问会提示你设置密码，设置好密码后即可登录后台。==请务必牢记密码==。

### 7. 配置旁路由

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-17.png)
...
![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-18.png)

PC 端可以直接在网络设置中修改网关地址。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-19.png)

以 Win10 为例，点击 WiFi 图标，点击对应 WiFi 下方的属性。

![alt text](images/飞牛OS虚拟机安装openwrt作为旁路由/image-20.png)
