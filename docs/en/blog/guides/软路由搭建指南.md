---
title: ImmortalWRT and OpenClash Router Setup Guide (Netcore N60 Pro)
description: A model-specific guide to backing up a Netcore N60 Pro, installing U-Boot and ImmortalWRT, configuring network access, and adding OpenClash for whole-home routing.
keywords:
  - software router
  - ImmortalWRT
  - OpenWrt
...
permalink: /en/blog/router-setup-guide/
lang: en-US
translationOf: /blog/router-setup-guide/
head:
  - - script
    ...
    - |
      {
        ...
        "@type": "HowTo",
        "name": "Install ImmortalWRT and OpenClash on a Netcore N60 Pro",
        "description": "Back up a Netcore N60 Pro, install its model-specific U-Boot and ImmortalWRT images, configure network access, and add OpenClash.",
        "totalTime": "PT30M",
        "step": [
          { "@type": "HowToStep", "name": "Obtain model-specific files", "text": "Download U-Boot and ImmortalWRT images that exactly match the device model and hardware revision." },
          { "@type": "HowToStep", "name": "Enable SSH", "text": "Connect by Ethernet, sign in to the stock interface, set an administrator password, and enable SSH." },
          { "@type": "HowToStep", "name": "Back up the stock flash", "text": "Inspect the device's actual MTD layout, back up the critical partitions, verify the files, and copy them to independent storage." },
          { "@type": "HowToStep", "name": "Install U-Boot", "text": "Only after verifying the exact device, image, checksum, partition name, and recovery path, upload and write the matching U-Boot image." },
          { "@type": "HowToStep", "name": "Install ImmortalWRT", "text": "Enter the U-Boot recovery interface and upload the matching ImmortalWRT firmware without interrupting power." },
          { "@type": "HowToStep", "name": "Configure networking and OpenClash", "text": "Secure the new interface, configure upstream access and Wi-Fi, install OpenClash and its core, import a private subscription, and test routing." }
        ]
      }
  - - script
    - type: application/ld+json
    - |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "How does an OpenWrt-capable router differ from a typical consumer router?",
            "acceptedAnswer": { "@type": "Answer", "text": "Many consumer routers use a restricted stock system. A supported OpenWrt or ImmortalWRT device can install additional packages such as policy-routing tools, but exact capabilities depend on the hardware and firmware build." }
          },
          {
            "@type": "Question",
            "name": "How does ImmortalWRT differ from OpenWrt?",
            "acceptedAnswer": { "@type": "Answer", "text": "ImmortalWRT is a downstream OpenWrt distribution with different packages, defaults, device support, and community focus. It is not universally better; choose the build that officially supports the exact hardware and required packages." }
          },
          {
            "@type": "Question",
            "name": "Can flashing a router brick it?",
            "acceptedAnswer": { "@type": "Answer", "text": "Yes. A wrong image, hardware-revision mismatch, incorrect MTD partition, power loss, or incomplete backup can make the device unbootable and may require serial access or an external programmer. Verify a model-specific recovery path before writing flash." }
          },
          {
            "@type": "Question",
            "name": "Why might the OpenClash menu not appear after installation?",
            "acceptedAnswer": { "@type": "Answer", "text": "The LuCI interface may need a new login or cache refresh, but a missing menu can also mean the package or dependencies failed. Check the package state and logs instead of assuming success." }
          },
          {
            "@type": "Question",
            "name": "Should the device be the primary router or a secondary router?",
            "acceptedAnswer": { "@type": "Answer", "text": "A primary router controls the full topology but requires migration. A secondary or repeater setup changes less, but can introduce double NAT, DNS, DHCP, roaming, and performance tradeoffs. Choose based on the existing network." }
          }
        ]
      }
---
...
![U-Boot download example](https://image.ermao.net/images/blog/router-setup-guide/20260524_150832-fb8fdc.png)
...
![ImmortalWRT firmware selection](https://image.ermao.net/images/blog/router-setup-guide/20260524_150844-f81af9.png)
...
![Open the stock administration interface](https://image.ermao.net/images/blog/router-setup-guide/20260524_150932-98cf90.png)

![Skip the stock Internet setup](https://image.ermao.net/images/blog/router-setup-guide/20260524_150945-af7962.png)

![Set the administrator password](https://image.ermao.net/images/blog/router-setup-guide/20260524_150955-7ff524.png)

![Enable SSH](https://image.ermao.net/images/blog/router-setup-guide/20260524_151033-aefa3c.png)
...
![SSH login](https://image.ermao.net/images/blog/router-setup-guide/20260524_151050-b91856.png)
...
![Inspect MTD partitions](https://image.ermao.net/images/blog/router-setup-guide/20260524_151118-3a27df.png)
...
![Check backup files](https://image.ermao.net/images/blog/router-setup-guide/20260524_151137-ec99fb.png)
...
![Export the backups](https://image.ermao.net/images/blog/router-setup-guide/20260524_151148-007f97.png)
...
![Write U-Boot](https://image.ermao.net/images/blog/router-setup-guide/20260524_151200-c0c503.png)
...
![U-Boot firmware page](https://image.ermao.net/images/blog/router-setup-guide/20260524_151235-6d1e20.png)

![Firmware update success page](https://image.ermao.net/images/blog/router-setup-guide/20260524_151243-bcbaa3.png)
...
![First ImmortalWRT login](https://image.ermao.net/images/blog/router-setup-guide/20260524_151255-81a4a9.png)

![Set the ImmortalWRT administrator password](https://image.ermao.net/images/blog/router-setup-guide/20260524_151309-200d38.png)
...
![Scan for the upstream Wi-Fi](https://image.ermao.net/images/blog/router-setup-guide/20260524_151341-49b103.png)

![Set wireless password 1](https://image.ermao.net/images/blog/router-setup-guide/20260524_151350-e02322.png)

![Set wireless password 2](https://image.ermao.net/images/blog/router-setup-guide/20260524_151358-e50b81.png)
...
![Refresh the package list](https://image.ermao.net/images/blog/router-setup-guide/20260524_151456-51d3a4.png)

![Install OpenClash](https://image.ermao.net/images/blog/router-setup-guide/20260524_151505-8215ae.png)
...
![Download a compatible core](https://image.ermao.net/images/blog/router-setup-guide/20260524_151517-dc0a3c.png)

![Review installation logs](https://image.ermao.net/images/blog/router-setup-guide/20260524_151526-d4bc39.png)

In the profile subscription section, add the provider subscription URL and update the profile list.

![Import an OpenClash profile](https://image.ermao.net/images/blog/router-setup-guide/20260524_151533-3286d1.png)
...
![Start OpenClash](https://image.ermao.net/images/blog/router-setup-guide/20260524_151544-5d907e.png)

Review the logs for errors:

![OpenClash startup log](https://image.ermao.net/images/blog/router-setup-guide/20260524_151554-89f78d.png)
